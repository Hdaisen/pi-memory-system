...
| `📄 convert_file` | 将 PDF/Word/Excel 等二进制文件转为 Markdown（需 WSL + MarkItDown） |
| `📦 ccr_retrieve` | 获取被自动压缩的原始内容（CCR 可逆压缩） |

## 自动功能

| 功能 | 触发时机 | 说明 |
|:-----|:---------|:------|
| 🔄 二进制文件转 Markdown | `read` 工具出错时 | PDF/DOCX/PPTX/XLSX → 自动转 Markdown |
| 📦 内容压缩 | `bash` 输出 >2KB 时 | JSON 数组 / 搜索结果 / 重复日志 → 紧凑格式 + CCR 缓存 |
| 🔙 原始内容找回 | LLM 主动调用 `ccr_retrieve` | 通过 `<<ccr:hash>>` 标记找回压缩前的完整内容 |
...
| 功能 | 依赖 | 安装方式 |
|:-----|:-----|:---------|
| 二进制文件转 Markdown | WSL + `markitdown` (Python) | `python3 -m venv ~/.markitdown-venv && ~/.markitdown-venv/bin/pip install 'markitdown[pdf]'` |
| 内容压缩 | 无（纯 TypeScript，零依赖） | 内置 |
...
│   └── compress.ts          # 内容压缩模块（被 memory.ts import）
├── templates/               # 模板文件
├── example/                 # 示例项目
├── scripts/                 # 初始化脚本
├── LICENSE                  # MIT
