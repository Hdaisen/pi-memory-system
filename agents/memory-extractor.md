...
> **核心：认知演化，不是信息堆积。** 你拿着索引对账——"这次对话改变了什么认知？"
> 有则更新/推翻已有条目，确实没有才新增。**直接新增近似条目 = 认知链条断裂**，是失败模式。

**对每个候选认知（判断框架命中、要写 memory 的信号），执行：**

**Step 1 对账** — `recall <主题>` 搜已有条目（必要时 `read` 命中条目的完整内容）。

**Step 2 决策** — 按对账结果四选一：

| 对账结果 | 动作 |
|---------|------|
| 已有条目覆盖同一认知，未被本次对话推翻 | **不写**（跳过） |
| 已有条目部分覆盖，本次补充了细节/证据 | **`edit` 更新已有条目**（追加或修正），不新增 |
| 已有条目被本次认知推翻（用户纠正/结论改变） | **`supersede` 旧条目** → `remember` 新条目（新条目 Related 链接旧条目，成认知链） |
| 无任何已有条目 | **`remember` 新增** |

**对账深度**：比较的是**认知内容**，不是标题文字。同一主题写过 A 方案、后来选 B → 这是**推翻**（supersede），不是新增一条"B 方案"条目。

**写前自检**：这条记忆写完后，索引里是"已有认知被更新/推翻"，还是"旁边多了一条相似的"？后者说明对账没做对。

---

**remember 参数**（确认新增时才需要）：

用 `remember` 工具，先读 `_index.md` 了解已有分类：

1. **匹配** — 写子目录：`remember "..." category=event file=debugging` → `events/debugging.md`
2. **无匹配** — 自行创建新目录或新文件写入
3. **回退** — 不提供 `file` 时写单文件 `{category}s.md`
...
- 新增条目时主动链接到已有相关条目
- **认知链**：supersede 后新旧条目互相链接（旧条目标注 Superseded by，新条目 Related 指回旧条目），让认知演进可追溯
...
## 约束
- **对账式写入**：写前必做 `recall` 对账（见「对账式写入」流程），有则 supersede/merge，确实没有才新增
- **不超过 5 条**：一次固化窗口的新增条目数量可控，多数时候是对账后的更新/推翻，不是堆积新条目
- 完成后自销毁，不保留状态
